...
import { Search as SearchIcon, Film, BookOpen, Tv, Star, Calendar, Plus } from 'lucide-react';
  ...
  return (
    <div className="container mx-auto px-4 py-8 mt-16 space-y-8">
      <div className="text-center space-y-4">
        <h1 className="text-4xl font-bold cinema-text">Search Movies, TV & Anime</h1>
      ...
      {(movies.length > 0 || tv.length > 0 || anime.length > 0) && (
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="tv" className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {tv.map((tvItem: any) => (
                <Card key={tvItem.id} className="glass-card hover-glow group cursor-pointer">
                  <CardContent className="p-0">
                    <div onClick={() => navigate(`/tv/${tvItem.id}`)}>
                      <img
                        src={tvItem.poster_path 
                          ? `https://image.tmdb.org/t/p/w500${tvItem.poster_path}` 
                          : '/placeholder.svg'
                        }
                        alt={tvItem.name}
                        className="w-full h-64 object-cover rounded-t-lg"
                      ...
                      <div className="p-4 space-y-2">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-2">
                            <Calendar className="w-4 h-4 text-gray-400" />
                            <span className="text-sm text-gray-400">
                              {tvItem.first_air_date ? new Date(tvItem.first_air_date).getFullYear() : 'N/A'}
                            </span>
                          </div>
                          {tvItem.vote_average > 0 && (
                            <div className="flex items-center space-x-1">
                              <Star className="w-4 h-4 text-yellow-400 fill-current" />
                              <span className="text-sm text-gray-300">{tvItem.vote_average.toFixed(1)}</span>
                            </div>
                          )}
                        </div>
                        <div className="flex flex-wrap gap-1">
                          {tvItem.genres?.slice(0, 2).map((genre: any) => (
                    ...
                    <div className="p-4 pt-0">
                      <Button
                        onClick={() => addToWatchlist('tv', tvItem.id.toString())}
      ...
      {!isLoading && query && movies.length === 0 && tv.length === 0 && anime.length === 0 && (
        <div className="text-center py-12">
